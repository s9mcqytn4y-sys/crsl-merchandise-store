...
export default function Invoice({ pesanan }: InvoiceProps) {
    ...
    // Integrasi Hook Polling Realtime Status Pembayaran
    const {
        ...
        activeOrder.status || "belum_bayar",
        {
            ...
            onSettled: (statusBaru) => {
                toast.success(
                    "🎉 Pembayaran berhasil dikonfirmasi! Pesanan Anda sedang dipersiapkan.",
                    { duration: 5000 }
                );
            },
        }
    ...
    return (
        <StorefrontLayout>
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        <span className="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md border border-emerald-200/60 inline-flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" /> FAKTUR RESMI PESANAN
                        </span>
            ...
            <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
                {/* Banner Sukses Pembayaran Lunas */}
                {isSettled && statusPesanan !== "dibatalkan" && (
                    <div className="bg-emerald-50 border border-emerald-200 text-emerald-950 p-6 rounded-3xl shadow-sm space-y-4 print:hidden">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3.5">
                                ...
                                <div>
                                    <h3 className="font-black text-base sm:text-lg text-emerald-900 tracking-tight">
                                        Pembayaran Terkonfirmasi Lunas!
                                    </h3>
                                    <p className="text-xs text-emerald-700 mt-0.5">
                                        Pesanan Anda sedang dipersiapkan dan diproses oleh tim fulfilment CRSL Official Store.
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2.5 w-full sm:w-auto">
                                <Link
                                    ...
                                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-sm"
                                >
                                ...
                                <Link
                                    href="/katalog"
                                    className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 bg-white border border-emerald-300 hover:bg-emerald-100/50 text-emerald-800 font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-2xs"
                                >
